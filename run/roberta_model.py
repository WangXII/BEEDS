...
class RobertaModelForQuestionAnswering(RobertaPreTrainedModel):
    r"""
        RobertaModelforQuestionAnswerTagging
    """
    def __init__(self, config, crf_bool):
        super(RobertaModelForQuestionAnswering, self).__init__(config)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None):
        with autocast():
            ...
            # logger.warning(torch.max(input_ids))
            # logger.warning(torch.max(attention_mask))
            # logger.warning(torch.min(attention_mask))
...
class ModelForDistantSupervision2(nn.Module):
    ...
    def __init__(self, config, multi_instance_bool, multi_instance_bool_neg, crf_bool, model_name):
        super(ModelForDistantSupervision2, self).__init__()
        self.multi_instance_bool = multi_instance_bool
        self.multi_instance_bool_neg = multi_instance_bool_neg
        self.roberta = RobertaModelForQuestionAnswering.from_pretrained(model_name, config=config, crf_bool=crf_bool)

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None):
        with autocast():
            sequence_logits, active_sequence_logits, logits, crf_sequence = self.roberta(
