...
from configs import STANDOFF_DIR, STANDOFF_CACHE, STANDOFF_CACHE_SIMPLE, SIMPLE_NORMALIZER_DB
...
class BaselineEventDict():
    ...
    def __parse_binding_pairs(self, ent_list):
        ...
        binding_ent = set()
        for ent in ent_list:
            ent_id = self.__parse_ent(ent)
            if ent_id != "":
                binding_ent.add(ent_id)
    ...
    def __parse_theme_event(self, theme_ref_id):
        ...
        if theme_ref_id.startswith("E"):
            ...
            if (event["Type"] in PTMS) or (event["Type"] in OTHER):
                if "Theme" in event:
                    theme_id = self.__parse_ent(event["Theme"])
                if (event["Type"] in PTMS) and ("Site" in event):
                    ...
                    for site in sites:
                        ...
                        yield theme_id, event_type, res, pos
                else:
                    yield theme_id, event_type, res, pos
        elif theme_ref_id.startswith("T"):
            theme_id = self.__parse_ent(theme_ref_id)
    ...
    def __parse_event(self, event):
        ...
        if event["Type"] in REGULATIONS:
            ...
            if ("Theme" in event) and ("Cause" in event):
                for theme_id, theme_event_type, res, pos in self.__parse_theme_event(event["Theme"]):
                    for cause_pairs in self.__parse_complex_id(event["Cause"]):
                        if isinstance(cause_pairs, tuple):
                            cause_id_0 = cause_pairs[0]
                            cause_id_1 = cause_pairs[1]
                        else:
                            cause_id_0 = cause_pairs
                            cause_id_1 = ""
                        if (cause_id_0 != "") and (theme_id != "") and (theme_event_type not in REGULATIONS):
                            ...
                            if cause_id_0 != "" and theme_id != "" and theme_event_type in PTMS:
                                db_value = (theme_id, cause_id_0, cause_id_1, event["Confidence #"], event["Negation"],
            ...
            if ("Theme" in event) and ("Site" in event):
                ...
                for site in sites:
                    ...
                    if theme_id != "":
                        ...
                        db_value = (theme_id, event["Confidence #"], event["Negation"], event["Speculation"], event["Type"], res, pos)
                        yield db_key, db_value

        elif event["Type"] == "Binding":
            binding_pairs = self.__parse_binding_pairs(event["Theme"])
            for binding_pair in binding_pairs:
                if len(binding_pair) < 2 and binding_pair[0] != "":
                    yield db_key, db_value
                elif binding_pair[0] != "" and binding_pair[1] != "":
                    db_key = event["Type"] + "_" + binding_pair[0]
                    db_value = (binding_pair[0], binding_pair[1], event["Confidence #"], event["Negation"], event["Speculation"], event["Type"])
                    yield db_key, db_value
                    db_key = event["Type"] + "_" + binding_pair[1]
                    db_value = (binding_pair[1], binding_pair[0], event["Confidence #"], event["Negation"], event["Speculation"], event["Type"])
                    yield db_key, db_value
...
if __name__ == "__main__":
    evex_dict = BaselineEventDict(STANDOFF_CACHE_SIMPLE, True)
